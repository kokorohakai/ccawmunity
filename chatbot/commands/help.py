...
def botHelp(body={}, roomId="", sender="", event={}):
    if( len(body) >= 2):
        ...
        return(bot.config.matrix["prefix"]+"help needs at least one argument - the name of another command, ie \""+bot.config.prefix+"help echo\"" \
                "\nFor a list of available commands try \""+bot.config.matrix["prefix"]+"commands\".")
